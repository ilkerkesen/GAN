...
function parse_options(args)
    s = ArgParseSettings()
    s.description = "Conditional DCGANs on MNIST."
...
function clip(x0, minval, maxval)
    x1 = max.(minval, x0)
    x2 = min.(maxval, x1)
end
...
splitdir(PROGRAM_FILE)[end] == "conditional-wasserstein-dcgan.jl" && main(ARGS)
