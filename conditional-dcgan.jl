...
function main(args)
    ...
    G = Generator(o[:zdim], o[:embed], o[:atype])
    ...
    for epoch = 1:o[:epochs]
        ...
        @time for (x,y) in dtrn
            ...
            dlossval += train_discriminator!(D,G,input,y,noise)
            noise = sample_noise(o[:atype],o[:zdim],length(y))
            glossval += train_generator!(G,D,noise,y)
        end
        ...
        if o[:outdir] != nothing
            ...
            plot_generations(G, l1; z=z, savefile=filepath,
...
function parse_options(args)
    ...
    @add_arg_table s begin
        ...
        ("--embed"; arg_type=Int; default=100; help="embedding dimension")
...
function (model::Discriminator)(x, c; alpha=0.2, training=true)
    y = vcat(model.embed(c), reshape(x, 784, size(x,4)))
    y = reshape(y, 28, 28, 2, size(x,4))
...
function Discriminator(atype)
    embed = Embedding(10, 784; atype=atype)
    dlayer1a = DLayer1(2, 20, 5; atype=atype)
    dlayer1b = DLayer1(20, 50, 5; atype=atype)
    dlayer2a = DLayer2(800, 500; atype=atype)
    softlayer = Linear(500, 1; atype=atype)

    return Discriminator(embed, dlayer1a, dlayer1b, dlayer2a, softlayer)
end


function dloss(D::Discriminator, real_images, fake_images, ygold)
    yreal = D(real_images, ygold)
    real_loss = -mean(log.(yreal))
    yfake = D(fake_images, ygold)
...
function train_discriminator!(D, G, real_images, ygold, noise)
    fake_images = G(noise, ygold; training=true)
    nsamples = div(length(real_images), 784)
    J = @diff dloss(D, real_images, fake_images, ygold)
    for p in params(D)
        g = grad(J, p)
...
function (model::Generator)(z, c; training=true)
    y = vcat(z, model.embed(c))
    y = model.glayer1a(y; training=training)
...
function Generator(zdim, nembed, atype)
    embed = Embedding(10, nembed)
    glayer1a = GLayerA(zdim+nembed, 500; atype=atype)
    ...
    Generator(embed, glayer1a, glayer2a, glayer3b, glayer4c, glayer5b, deconv)
end


function gloss(G, D, noise, labels)
    fake_images = G(noise, labels)
    ypred = D(fake_images, labels)
    return -mean(log.(ypred))
end

function train_generator!(G, D, noise, labels)
    J = @diff gloss(G, D, noise, labels)
    for p in params(G)
        g = grad(J, p)
        update!(value(p), g, p.opt)
    end
...
function plot_generations(
    G, labels; z=nothing, gridsize=(8,8), scale=1.0, savefile=nothing)
    if z == nothing
        nimg = prod(gridsize)
        embed = size(G.embed, 1)
        w = G.glayer1a.deconv.w
        zdim = size(w, 2) - embed
        atype = w isa KnetArray ? KnetArray{Float32} : Array{Float32}
        z = sample_noise(atype,zdim,nimg)
    end
    output = Array(0.5 * (1 .+ G(z, labels; training=false)))
